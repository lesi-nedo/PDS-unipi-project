...
int main(int argc, char* argv[]) {
    ...
    printf("Which FastFlow Decision Forest implementation should be used? [ff_exp1/ff_exp2]\n");
    std::string implementation_choice;
    std::cin >> implementation_choice;
    if (implementation_choice != "ff_exp1" && implementation_choice != "ff_exp2") {
        std::cerr << "Invalid choice. Please choose either 'ff_exp1' or 'ff_exp2'." << std::endl;
        return 1;
    }
    ...
    std::string results_path = std::filesystem::absolute("./results/validation/ff_sklearn/" + implementation_choice + "/");
    std::ofstream path_file(std::filesystem::absolute("./results/validation/ff_sklearn/path.txt"));
    if (path_file.is_open()) {
        path_file << results_path;
        path_file.close();
    } else {
        std::cerr << "Error: Unable to open path file for writing." << std::endl;
        return 1;
    }
    ...
    ff_ml_exp1::DecisionForest<double, int, double> rf_exp1;
    ff_ml_exp2::DecisionForest<double, int, double> rf_exp2;
    ...
    for(size_t idx = 0; idx < samples_perTree.size(); ++idx) {
        auto samples_perTree_i = samples_perTree[idx];
        ...
        for(auto& tree_counts_i : tree_counts) {
            ...
            if (implementation_choice == "ff_exp1") {
                ...
                rf_exp1.learn(features_train, labels_train, tree_counts_i, randomSeed);

                

                std::cout << "Predicting with " << tree_counts_i << " trees." << std::endl;
                rf_exp1.predict(features_test, predictions);
            ...
            try {
                savePredictionsToFile(
                    classPredictions,
                    results_path + "ff_predictions_" + std::to_string(tree_counts_i) + "_trees_" + 
                    std::to_string(samples_perTree_i) + "_" + std::to_string(samples_perTree_test[idx]) + "_samples.csv"
                );
                saveProbabilitiesToFile(
                    predictions,
                    results_path + "ff_probabilities_" + std::to_string(tree_counts_i) + "_trees_" + 
                    std::to_string(samples_perTree_i) + "_" + std::to_string(samples_perTree_test[idx]) + "_samples.csv"
                );
            ...
            for (size_t i = 0; i < std::min(static_cast<size_t>(10), predictions.shape(0)); ++i) {
                std::cout << std::setw(6) << i << " | "
                          ...
                          << std::setw(9) << classPredictions[i] << " | " << std::setw(6);
